...
touchScroll.draw = function(){
  ...
  for( var i = 0; i < this.frame.pointables.length ; i++){
    ...
    if(i == 0 ){
      ...
      if( this.frame.pointables[0].touchZone == 'touching' ){
        if( this.scrollInX ){
          this.speed[0] += this.frame.pointables[0].tipVelocity[0]/500;
        }
        if( this.scrollInY ){
          this.speed[1] += this.frame.pointables[0].tipVelocity[1]/500;
        }
      }
    }else{
